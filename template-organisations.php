 ...
 *
 * "Chosen by over 145 organisations" showcase page.
 *
 * Layout:
...
/* ── ACF fields ── */
$hero_eyebrow     = trim((string) (get_field('org_hero_eyebrow') ?: __('Accessibility & Learning', 'elahub')));
$hero_icon        = trim((string) (get_field('org_hero_icon') ?: 'fa-solid fa-universal-access'));
$hero_heading     = trim((string) (get_field('org_hero_heading') ?: get_the_title()));
$hero_description = trim((string) (get_field('org_hero_description') ?: ''));
$intro_text       = trim((string) (get_field('org_intro_text') ?: ''));
$organisations    = get_field('org_items') ?: [];

/* ── Group organisations by first letter for optional alpha sections ── */
// Flat list — alphabetical sort by name
usort($organisations, fn($a, $b) => strcasecmp($a['org_name'] ?? '', $b['org_name'] ?? ''));
?>
...
<main id="primary" class="site-main">
        ...
        <div class="container py-10 md:py-14">
            ...
            <p class="!mb-8 !text-sm text-text/50">
                ...
                echo esc_html(sprintf(
                    /* translators: %d = number of organisations */
                    _n('%d organisation', '%d organisations', count($organisations), 'elahub'),
                    count($organisations)
                ));
                ?>
            </p>
            ...
            <ul class="m-0 grid list-none grid-cols-2 gap-5 p-0 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5">
                ...
                <?php foreach ($organisations as $org) :
                    $name = trim((string) ($org['org_name'] ?? ''));
                    $url  = trim((string) ($org['org_url'] ?? ''));
                    $logo = $org['org_logo'] ?? null;
                    ...
                    $logo_alt = is_array($logo) ? ($logo['alt'] ?? $name) : $name;
                    if (! $logo_alt) {
                        $logo_alt = $name;
                    }
                ...
                ?>
                    <li>
                        <<?php echo $tag; ?> <?php echo $attrs; ?>
                            ...
                            <?php if ($logo_src) : ?>
                                <?php /* Logo — fixed responsive height so all cards are uniform across the row */ ?>
    ...
    <?php else : ?>
        <div class="container py-16 text-center">
            <p class="!mb-0 text-text/50"><?php esc_html_e('No organisations listed yet.', 'elahub'); ?></p>
        </div>
    <?php endif; ?>
